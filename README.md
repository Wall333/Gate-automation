...
- **Push notifications:** Delivered via Expo Push API with Firebase Cloud Messaging (FCM) as the Android transport. The server sends a simple HTTP POST to Expo's push service — no Firebase Admin SDK needed. `google-services.json` is required in the Android build but excluded from version control.
